# 3027. Find the Number of Ways to Place People II  
  ...
  Methods: Sort </br> Difficulty: Hard </br> </br>You are given a 2D array `points` of size `n x 2` representing integer coordinates of some points on a 2D-plane, where `points[i] = [xi, yi]`.
...
class Solution {
    public int numberOfPairs(int[][] P) {
        ...
        int ans = 0;
        for (int i = 0; i < n - 1; i++) {
            int y = Integer.MAX_VALUE;// 目前能接受的 最大 Bob.y 上界
            int yi = P[i][1];
            for (int j = i + 1; j < n; j++) {
                int yj = P[j][1];
